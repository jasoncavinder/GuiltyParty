...
The browser Host and Companion fallback under `apps/web/`, and the legacy Stage
proof under `clients/stage/`, currently have no package-manager manifest and
load no third-party script, stylesheet, font, or media asset from an external
origin. Their application visuals are first-party HTML and CSS. This statement
must be rechecked from the built artifact before external distribution.
